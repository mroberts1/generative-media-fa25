...
<div class="shader-wrap">
  <canvas id="sp-canvas" style="width:100%;height:100%;display:block;border-radius:8px;"></canvas>
</div>

<script src="https://unpkg.com/shader-park-core@0.1.41/dist/shader-park-core.js"></script>
<script>
  (function(){
    const container = document.querySelector('.shader-wrap');
    const canvas = document.getElementById('sp-canvas');

    function sizeCanvas() {
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      // Ensure client sizes exist (fallback to container bounds)
      const w = canvas.clientWidth || container.clientWidth || 500;
      const h = canvas.clientHeight || container.clientHeight || 500;
    ...
    function getRenderer() {
      const w = window;
      if (w.shaderParkCore && typeof w.shaderParkCore.sculptToMinimalRenderer === 'function') return w.shaderParkCore.sculptToMinimalRenderer;
      if (w.shaderParkCore && w.shaderParkCore.default && typeof w.shaderParkCore.default.sculptToMinimalRenderer === 'function') return w.shaderParkCore.default.sculptToMinimalRenderer;
      if (w.ShaderPark && typeof w.ShaderPark.sculptToMinimalRenderer === 'function') return w.ShaderPark.sculptToMinimalRenderer;
      if (typeof w.sculptToMinimalRenderer === 'function') return w.sculptToMinimalRenderer;
      return null;
    }
    ...
    const init = () => {
      ...
      const code = `
// T1 by h2ee - adapted for local rendering
let pos = getSpace();
...
for(let i = 3; i < 6; i++) {
  for(let j = 1; j < 6; j++) {
    rotateX(time*0.1*${SPEED}*j*i);
    rotateY(time*0.1*${SPEED}*j*i);
...
let oscillation = abs(sin(time*${SPEED}));
let endSize = 0.9;
...
`;
      ...
      try {
        ...
        if (render) {
          ...
          console.error('Shader Park renderer not found');
        }
      } catch (e) {
        console.error('Shader Park init error:', e);
      }
    };

    // Lazy init when in view
...
- Excerpts from *Sisters With Transistors* (documentary) (Lisa Rovner, 2022)
